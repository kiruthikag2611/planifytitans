...
function ActivitiesView() {
    ...
    const upcomingActivities = useMemo(() => activities?.filter(a => isFuture(new Date(a.startDatetime))) || [], [activities]);
    const pastActivities = useMemo(() => activities?.filter(a => isPast(new Date(a.startDatetime))) || [], [activities]);
...
function ActivityCard({ activity, onClick }: { activity: Activity, onClick: (activity: Activity) => void }) {
    const isOngoing = new Date() >= new Date(activity.startDatetime) && new Date() <= new Date(activity.endDatetime);
    const badgeText = isOngoing ? 'Ongoing' : isToday(new Date(activity.startDatetime)) ? 'Today' : `in ${formatDistanceToNow(new Date(activity.startDatetime))}`;
            ...
            <CardContent className="p-4 flex gap-4">
                ...
                <div className="flex-grow">
                    ...
                    <p className="text-sm text-muted-foreground flex items-center gap-2">
                        ...
                        {format(new Date(activity.startDatetime), 'EEE, MMM d, p')} - {format(new Date(activity.endDatetime), 'p')}
                    </p>
